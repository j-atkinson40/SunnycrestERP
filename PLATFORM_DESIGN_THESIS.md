...
| [`BRIDGEABLE_MASTER.md`](BRIDGEABLE_MASTER.md) | All three | Strategy, vision, roadmap, vertical decomposition. **§2.4 Vertical-as-Workflow-Composition Principle** is the architectural foundation underneath the three design layers — verticals are workflow compositions of universal primitives, not bespoke vertical-specific code. The composition principle is platform-architecture (how the platform actually works), distinct from but complementary to the three-layer design thesis (how the platform looks + behaves + feels). |
...
**Architectural foundation note** (added at §2.4 canon batch write): the three-layer design thesis (Range Rover / Tony Stark / Apple Pro) describes the platform's design identity. BRIDGEABLE_MASTER.md §2.4 describes the platform's architectural identity — the structural claim that verticals are workflow compositions over universal primitives. The two are orthogonal canonical articulations: design thesis answers "how should it look + behave + feel"; composition principle answers "how is it structurally built such that vertical expansion economics are tractable across the long arc." Both are canonical; both are load-bearing. Neither is reducible to the other. The composition principle is structurally significant for technical audiences + sophisticated business operators + strategic partners; the design thesis is sensorially significant for every operator. Bridgeable's *visual design* explicitly does NOT look like Notion (per Layer 1 + Layer 2 + Layer 3 rejections); Bridgeable's *primitive composition architecture* IS in the same family as Notion's per §2.4.7 — two complementary canonical articulations.

**Portal extension foundation note** (added at §2.5 canon batch write): BRIDGEABLE_MASTER.md §2.5 Portal Extension Pattern is sibling platform-thesis-foundation canon at §2.4-parallel depth. Where §2.4 canonicalizes vertical expansion (verticals are workflow compositions of universal primitives), §2.5 canonicalizes portal expansion (portals are Spaces configured with portal-specific constraints, not net-new substrate). Together §2.4 + §2.5 establish canonical-substrate-extension canon at platform-thesis-foundation depth — both vertical and portal expansion configure canonical substrate, neither constructs net-new architectural substrate. Design thesis (this doc) is orthogonal to both — design canon governs how surfaces look + behave + feel regardless of whether surfaces are tenant office Spaces, tenant operational-role Portal Spaces, or non-tenant-user Portal Spaces. Portal-Space tenant-branding "wash, not reskin" discipline (per SPACES_ARCHITECTURE.md §10.6) preserves design thesis canonical-coherence — portal Spaces inherit canonical visual canon at DESIGN_LANGUAGE §13 + §14 + tenant-branding wash overlay only.
