...
# ── Session 3 — the declaration gates opening ────────────────────────


def test_openable_is_gated_on_the_widget_declaring_peek_inline():
    """`openable` is not "is it a peek target" — every standing entry is.

    It is "does this widget declare it can BE peek panel content". The
    distinction is load-bearing: `ar_summary` is a real standing entry
    whose renderer was never built, and it declares `dashboard_grid`
    alone. Gating on the declaration makes it decline the panel instead
    of opening onto "Widget unavailable".
    """
    from app.api.routes.note import _PEEK_INLINE_WIDGET_IDS

    # The four session-3 targets opted in.
    for widget_id in (
        "today",
        "vault_schedule",
        "line_status",
        ...
        assert widget_id in _PEEK_INLINE_WIDGET_IDS, widget_id

    # ar_summary did NOT, and that is the mechanism working.
    assert "ar_summary" not in _PEEK_INLINE_WIDGET_IDS


def test_ar_summary_is_still_a_real_standing_entry():
    """Guards the difference between "declines the panel" and "was removed".

    If a future change drops `ar_summary` from the accountant's template,
    that template becomes EMPTY — it is the only entry. This test fails
    loudly in that case rather than letting the emptiness pass as a
    successful cleanup.
    """
