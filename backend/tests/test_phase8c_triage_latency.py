...
#: tests/_gc_latency.py. Measured 2026-09-15 over 3-4 runs, spread <= 1, x3.
#:
#: ⚠️ PER QUEUE, NOT SHARED. ar-collections and expense-categorization allocate
#: an order of magnitude more per request than month-end-close, so one number
#: for the file would be loose enough on two queues to assert nothing on the
#: third. A KeyError on a queue added without a measurement is deliberate.
_ALLOC_CEILINGS: dict[str, int] = {
    "month-end-close-next-item": 10,
    "month-end-close-apply-action": 20,
    "ar-collections-next-item": 100,
    "ar-collections-apply-action": 60,
    "expense-categorization-next-item": 100,
    "expense-categorization-apply-action": 60,
}
