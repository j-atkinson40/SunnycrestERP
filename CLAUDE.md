...
- A standing line opens its Focus on click — read-only for a user without edit permission. Every entry costs the same: one click to the full surface. A peek was tried and reverted; a panel that opens on cursor travel costs more than a click, and at peek size the schedule cannot be read anyway. ⚠️ This bullet asserted the peek-only rule until 2026-09-22. That rule was reversed on 2026-09-09 (`4cbfbd00`, reverting `3d21e919`) and the §1a rewrite re-asserted it from a ruled-items list without checking it against the reversal — so the document read every session carried the superseded rule while DECISIONS carried the live one
...
| [PLATFORM_ARCHITECTURE.md](PLATFORM_ARCHITECTURE.md) | Three-primitive architecture (Monitor / Act / Decide), Focus primitive, note surface, entity portal, chip conversation, pause sensor, observe-and-offer, bounded-decision discipline, cross-cutting principles. April 22, 2026. | Any architectural decision touching the new primitives |
...
**Scope relationship — BRIDGEABLE_MASTER vs PLATFORM_ARCHITECTURE.** Both are authoritative for their coverage areas, but they represent two layers of architectural thinking. `BRIDGEABLE_MASTER.md` captures the April 19, 2026 consolidation — three primitives of **Spaces / Saved Views / Pins**, the Command Bar with 18 unified capabilities, Bridgeable Assist, Vision, Knowledge Ingestion, Personalization Studio, N-way cross-tenant. `PLATFORM_ARCHITECTURE.md` (April 22, 2026) introduces a later architectural layer: the three primitives reframe as **Monitor (the daily note) / Act (Command Bar) / Decide (Focus)**, formalizes Focus as the bounded-decision primitive, introduces the note as the per-user, per-day Monitor surface, entity portal as the Command Bar organizing pattern, chip conversation + pause sensor as the multi-step input pattern, and observe-and-offer as the cross-cutting configuration principle.
...
- **UI-touching prompts:** CLAUDE.md + PLATFORM_DESIGN_THESIS.md + DESIGN_LANGUAGE.md + PLATFORM_INTERACTION_MODEL.md + PLATFORM_QUALITY_BAR.md + PLATFORM_ARCHITECTURE.md (for new primitives) or UI_UX_ARC.md (for shipped primitives). Add PLATFORM_PRODUCT_PRINCIPLES.md when the prompt touches scan-and-act surfaces, note composition, onboarding, or permission flows.
...
**Note:** `UI_UX_ARC.md` documents the seven primitives shipped during Phases 1–7 (Command Bar v1, Saved Views, Spaces, NL Creation, Triage, Briefings, Polish). It is NOT in the canonical-specs registry above because it is an arc log, not an architectural spec. `PLATFORM_ARCHITECTURE.md` supersedes its architectural framing (Command Bar v1 becomes the Act primitive; Spaces filter the note surface; Focus is the newly-introduced third primitive not in UI_UX_ARC).
...
| Should this be a triage queue, a saved view, or a note fragment? | `UI_UX_ARC.md` (triage, saved views) · `CLAUDE.md §1a` (note fragments) |
...
The four new ComponentKinds (`entity-card`, `button`, `form-input`, `surface-card`) join the existing 7 (`widget`, `focus`, `focus-template`, `document-block`, `workflow-node`, `layout`, `composite`); the migration extends both the `component_configurations.component_kind` CHECK constraint and the new `component_class_configurations.component_class` CHECK constraint to permit them.
...
- **Task surfaces** (list views, detail views, creation forms, note fragment declarations, briefings consumption, future authoring surfaces)
