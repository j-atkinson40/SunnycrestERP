...
- **Funeral Schedule kanban DeliveryCard** ([`frontend/src/components/dispatch/DeliveryCard.tsx`](frontend/src/components/dispatch/DeliveryCard.tsx)) — Phase 3.1 + 3.2 + 4.2.4 work. Whole-card drag, restrained typography hierarchy, accent (terracotta) for status emphasis only, no perimeter border. Cards float on the lane background as material objects. **Canonical card surface for the platform.**
...
- **AncillaryPoolPin** ([`frontend/src/components/dispatch/scheduling-focus/AncillaryPoolPin.tsx`](frontend/src/components/dispatch/scheduling-focus/AncillaryPoolPin.tsx)) — workshop-tool register at small scale. Whole-row drag, no handle, accent count chip. Compact rows, no decorative chrome.
- **DateBox flanking affordance (Phase 4.4.3)** ([`frontend/src/components/dispatch/scheduling-focus/DateBox.tsx`](frontend/src/components/dispatch/scheduling-focus/DateBox.tsx)) — first surface built against locked DESIGN_LANGUAGE Section 0. Square-shouldered, accent-on-active jewelry signal (terracotta), restrained at rest. Calibration surface for all three layers passing simultaneously.
...
- **Accent primary buttons platform-wide** (deepened terracotta) — single-color functional emphasis. Same button reads native to both light and dark mode. Material continuity across modes.
