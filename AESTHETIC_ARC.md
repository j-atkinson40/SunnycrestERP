...
### Phase A — Entity Model + Native-from-day-one Framework + Real-time Delivery + Retention + Storage + Cross-primitive Embedding (7 subsections)

§3.26.19 introduced as comprehensive Messaging Primitive section (20 subsections — tightest of five communication primitives proportional to Messaging's tighter architectural complexity). Messaging established as **fifth and final concrete Communications layer primitive** per Phase W-4b sequence step 5 (after Email Step 1 + Calendar Step 2 + SMS Step 3 + Phone Step 4; sequencing per §3.26.6.4). Phase A locks the entity model + native-from-day-one framework + real-time delivery infrastructure + retention + storage + polymorphic linkage + privacy/compliance + cost discipline reduction; Phase B locks surfacing + composition + cross-primitive embedding + strategic vision integration; Phase C locks Focus + Workshop + Intelligence + Visual canon integration.

**Architectural framing — Messaging as five-axis-distinct primitive** (§3.26.19): Messaging is **architecturally lighter than SMS** on substrate (no provider abstraction; no carrier compliance; no per-message cost), **architecturally lighter than Email** on transport complexity (no SMTP/IMAP/MIME; no HTML body sandbox; no spam/deliverability/bounce), **architecturally lighter than Calendar** on derived-artifact decomposition (no recurrence engine; no free/busy resolution; no iTIP processing), **architecturally lighter than Phone** on intelligence-as-primary-value framing (raw text body IS the primary surface; intelligence is enrichment not primary value generator), but **architecturally heavier on cross-primitive embedding** (messages embed entity references + saved view previews + email thread excerpts + calendar event references + SMS conversation snippets + call intelligence summaries — operational substrate connective tissue). **And architecturally distinct on native-from-day-one framing** — Bridgeable owns entire message flow; integrate-now-make-native-later canonical inverts at Messaging primitive.

**Native-from-day-one framework canonical** (inverts §3.26.15.1 + §3.26.16.1 + §3.26.17.1 + §3.26.18.1 integrate-now-make-native-later canonical): Bridgeable's Messaging primitive owns entire message flow from operator composition through real-time delivery through retention. Architectural rationale: no external transport need (internal-tenant + cross-tenant Bridgeable-user only); no regulatory compliance frameworks (employment relationship establishes consent baseline; cross-tenant bilateral consent only); no cost discipline at primitive level (zero per-message marginal cost); no provider ecosystem complexity. Native-from-day-one framework simplifies primitive — entity model lighter (4 entities vs SMS's 5+ vs Phone's 7); compliance discipline lighter; cost discipline lighter; provider abstraction non-existent.

**Operational coordination dimension distinct from prior four communication primitives**: Email = interpersonal coordination (external + internal recipients); Calendar = temporal coordination (scheduling); SMS = operational reach (mobile/customer-facing); Phone = operational intelligence (call intelligence pipeline). Messaging adds **operational coordination** — operator-to-operator coordination dimension distinct from prior primitives. Messaging is the platform's internal coordination substrate.

**Cross-primitive embedding as operational substrate connective tissue**: messages embed cross-primitive content via `body_blocks_json` structured embeds. Six canonical embed types at September scope per Phase A Q5 extension — entity mention + saved view + email thread + calendar event + SMS conversation snippet + call intelligence summary. Cross-primitive embedding establishes Messaging primitive as connective tissue across other primitives.

**Entity model canonical** (§3.26.19.2): four core entities — two primary (Thread + Message) + two supporting (ThreadParticipant + ThreadType discriminator). Single `message_threads` table with `thread_type` discriminator (`dm` / `channel`); per-type variation handled via discriminator + per-type rendering chrome + per-type retention policy lookup, not entity decomposition.

**Real-time delivery infrastructure canonical** (§3.26.19.3): WebSocket canonical primary + SSE alternate. Real-time scope at September lighter canonical: message delivery push + read-receipts + lightweight "Active in last 5 minutes" presence + unread count badges + web push notifications. Typing indicators + full presence + mobile push + email digest deferred per §3.26.19.20.

**Cross-tenant bilateral consent canonical** (§3.26.19.6 + §3.26.19.13) parallel to §3.26.16.6 calendar + §3.26.17.15 SMS + §3.26.18.17 phone bilateral consent precedent. **Bilateral consent pattern locks across all five Layer 1 communication primitives**.

**Storage retention discipline lighter than prior primitives** (§3.26.19.4 + §3.26.19.7): per-thread-type differential retention canonical — DM threads 1-year default + Channel threads 2-year default; 90-day minimum + 7-year maximum tenant-configurable. **Cost discipline reduces to storage retention only at primitive level** — Messaging primitive's zero per-message marginal cost inverts SMS's per-message cost discipline + Phone's multi-dimensional cost discipline.

### Phase A — Q1-Q7 deliberations + 4 refinements integrated

Seven architectural questions surfaced + resolved with four refinements integrated:

- **Q1 native-from-day-one framework** confirmed (inverts integrate-now-make-native-later canonical of prior four primitives); future external messaging integration (Slack/Teams bidirectional) deferred per §3.26.19.20 with three documented signals canonicalized
- **Q2 single message_threads table with thread_type discriminator** confirmed (dm / channel); single-entity-with-discriminator chosen over decomposed for architectural simplicity
- **Q3 real-time delivery scope at September lighter canonical** confirmed (message delivery push + read-receipts + lightweight presence + unread count + web push); typing indicators + full presence + mobile push + email digest deferred with concrete signals canonicalized (Phase A refinement integrated)
- **Q4 cross-tenant messaging bilateral consent canonical** confirmed (parallel to §3.26.16.6 + §3.26.17.15 + §3.26.18.17 cross-tenant bilateral consent precedent); pattern locks across all five communication primitives
- **Q5 cross-primitive embedding scope canonical at September** confirmed with extension — SIX canonical embed types at September scope (added sms_conversation embed type per Q5 extension; Phase A refinement integrated)
- **Q6 per-thread-type differential retention canonical** confirmed (DM 1-year default + Channel 2-year default; 90-day min + 7-year max tenant-configurable)
- **Q7 cost discipline reduces to storage retention only canonical** confirmed at primitive level; per-channel storage cost attribution deferred per §3.26.7.5 with concrete signal documented (Phase A refinement integrated)

### Phase B — Surfacing + Composition + Cross-primitive Embedding + Strategic Vision Integration (8 subsections)

§3.26.19.8-§3.26.19.15. Three-shape surface decomposition: unified messaging surface (messaging-workspace-shape at `/messages` distinct from Space architecture per §3.26.19.8) + Pulse Messaging rendering across multiple layers (Communications + Operational layers per §3.26.19.9 hybrid contribution pattern) + cross-primitive embedding rendering at §14.13.5.
...
**Hybrid layer contribution pattern canonical** (§3.26.19.9, parallel to §3.26.16.10 calendar + §3.26.17.11 SMS + §3.26.18.12 Phone): Messaging contributes to MULTIPLE Pulse layers — Communications layer (DM activity + cross-tenant thread activity) + Operational layer (operational channels + operational mentions). **Pulse layer hybrid contribution canon canonical-quality complete with Messaging primitive canonicalization**. Pattern locks finally across all five Layer 1 communication primitives.

**Communications + scheduling + reach + intelligence + coordination = operations strategic framing locked** (§3.26.19.14, five-part operational substrate completion): Messaging primitive's strategic role completes operational substrate five-part composition. Three architectural justifications preserved at canon level (coordination is genuinely Messaging-distinctive among Layer 1 communication primitives — Email + SMS + Phone serve external-facing + customer-facing communication; Calendar serves temporal coordination; Messaging serves operator-to-operator internal coordination + cross-tenant operator coordination + cross-primitive embedding as operational substrate connective tissue).

**Concrete competitor comparison preserved at canon level**: standalone team messaging platforms (Slack + Microsoft Teams + Discord) charge per-seat licensing ($7-25/seat/month); Bridgeable's Messaging-as-platform-primitive framing eliminates dual context. Strategic differentiation completes operational substrate communication primitives.

**September Wilbert demo concrete scenario locked at canon level** (Phase B strengthening integrated): demo can demonstrate operational state coherence across all five communication primitives — single operational state with five communication primitive surfaces. Sunnycrest dispatcher coordinates production with foreman via #production-floor channel; channel message embeds today's production schedule saved view; mention `@order:SO-2026-0142` resolves to Hopkins service order; embed Hopkins email thread for context; cross-tenant DM with Hopkins driver coordinates pickup time. Five primitives + one operational state — structurally inaccessible to fragmented competitor stacks (Slack + Salesforce + Calendly + Twilio + Gong).

**Cross-primitive embedding rendering** per §14.13.5: each embed type renders as Pattern 2 sub-card with primitive-specific chrome continuity per Q11 confirmation. Primitive-specific chrome continuity preserves cross-primitive cognitive coherence — operator viewing message with calendar event embed sees calendar event chrome they recognize from `/calendar` workspace; cross-primitive embedding feels operational not bolt-on.

### Phase B — Q8-Q11 deliberations + 1 strengthening integrated

Four architectural questions surfaced + resolved with one strengthening integrated:

- **Q8 separate /messages route canonical** confirmed (workspace-shape distinct from Spaces architecture; split-pane composition canonical); workspace-shape distinction pattern locks finally across all five Layer 1 communication primitives
- **Q9 hybrid contribution pattern** confirmed (Communications + Operational layers split; pattern locks finally across all five Layer 1 communication primitives)
- **Q10 "Communications + scheduling + reach + intelligence + coordination = operations"** confirmed canonical strategic framing; three architectural justifications preserved at canon level; concrete competitor comparison (Slack + Microsoft Teams + Discord) preserved at canon level; **Wilbert demo concrete scenario added as canonical demonstration of five-primitive + one-operational-state cumulative architectural elegance** (Phase B strengthening integrated)
- **Q11 primitive-specific chrome continuity at §14.13.5** confirmed canonical; unified embed chrome alternative rejected — cognitive coherence wins over visual consistency

### Phase C — Focus + Workshop + Intelligence Integration + Visual Canon (5 BRIDGEABLE_MASTER subsections + 6 DESIGN_LANGUAGE subsections)

§3.26.19.16-§3.26.19.20 + §14.13. Messaging-triggered Decision Focus integration ships at canon-faithful depth — three canonical triage queue config types at September scope (`messaging_stale_thread_review_triage` + `messaging_cross_tenant_acknowledgment_triage` + `messaging_mention_review_triage`); lighter than Phone's 6 + SMS's 5 — proportional to Messaging's tighter compliance + cost discipline. Phase 5 triage primitive substrate enables implementation when Phase W-4b Messaging Step ships triage integration.

**Messaging-mediated Coordination Focus deferred** — five concrete signals; deferral is **symmetric across email-mediated AND calendar-mediated AND SMS-mediated AND phone-mediated AND messaging-mediated Coordination Focus integration**; **single Coordination Focus arc unblocks all FIVE communication primitives' integration** when concrete signals warrant.

**Messaging Intelligence at three-prompt September scope**: `messaging.priority_classification` (Haiku) + `messaging.summarize_thread` (Haiku) + `messaging.intent_classify` (Haiku). All Haiku per fast-classification + fast-summarization requirement. Lighter than Phone's 7-prompt scope + Email's 7-prompt scope — proportional to Messaging's intelligence-as-enrichment framing. Three deferred prompts per §3.26.19.20 with concrete signals documented (`messaging.action_item_extract` + `messaging.summarize_thread` Sonnet upgrade + `messaging.sentiment_classify`).

**Messaging Workshop at three-template-type granularity** (Q13 confirmation): three distinct Workshop template types — `message_template` + `automated_reply_rule_template` + `channel_rule_template`. Meta-pattern continuity preserved — Workshop template granularity is determined by structural-overlap-of-composition-shapes per §3.26.16.24 calendar precedent. Messaging's 3 template types matches Calendar's 3 template type count — proportional to lighter primitive complexity.

**Total cross-primitive Workshop template types canonical at canon level (post-Session-6)**: Email 1 + Calendar 3 + SMS 5 + Phone 7 + Messaging 3 = **19 Workshop template types**. **Meta-pattern locks finally across all five Layer 1 communication primitives**. Workshop template granularity is determined by structural-overlap-of-composition-shapes per established meta-pattern.

**Messaging Visual Canon extends §14 Communications Visual System** (Q12): §14.13 ships six subsections matching §14.9 Email + §14.10 Calendar + §14.11 SMS + §14.12 Phone precedent — §14.13.1 Messaging thread list (split-pane workspace + DM/channel/cross-tenant section grouping) + §14.13.2 Thread detail rendering (DM + channel composition variants) + §14.13.3 Messaging composition surface (inline + new DM modal + new channel modal) + §14.13.4 Cross-tenant messaging chrome + §14.13.5 Cross-primitive embedding chrome (six embed types with primitive-specific chrome continuity) + §14.13.6 Cross-references. **§14.13 is the final §14 extension — cross-primitive visual continuity locks finally across all five Layer 1 communication primitives.**

### Phase C — Q12-Q14 deliberations

Three architectural questions surfaced + resolved:

- **Q12 §14.13 extends §14 Communications Visual System** confirmed (six subsections matching §14.9 Email + §14.10 Calendar + §14.11 SMS + §14.12 Phone precedent); cross-primitive visual continuity locks finally across all five Layer 1 communication primitives
- **Q13 per-template-type with three template types** confirmed (message_template + automated_reply_rule_template + channel_rule_template); meta-pattern locks finally across all five Layer 1 communication primitives
- **Q14 3 managed prompts canonical at September scope** confirmed (priority_classification + summarize_thread + intent_classify; all Haiku per fast-classification + fast-summarization requirement; lighter than Phone's 7-prompt scope proportional to Messaging's intelligence-as-enrichment framing)

### Phase A + B + C refinements integrated

- 4 Phase A refinements: Slack/Teams bidirectional integration deferral signal (Q1); typing indicators + full presence + mobile push + email digest deferral signals (Q3); sms_conversation embed type extension (Q5; six canonical embed types total); per-channel storage cost attribution deferral signal (Q7)
- 1 Phase B strengthening: Wilbert demo concrete scenario prose locking five-primitive + one-operational-state cumulative architectural elegance demonstration at canon level (Q10)
- 14 architectural questions Q1-Q14 deliberated and resolved across three phases

**Architectural model post-Session-6**: 5 primitives + fractal Pulse + universal Command Bar + Workshop primitive + comprehensive Email + Calendar + SMS + Phone + Messaging primitive canon. Email + Calendar + SMS + Phone + Messaging = five communication primitives canonicalized at canon-faithful depth. Together they form the **operational substrate five-part composition** — interpersonal coordination + temporal coordination + operational reach + operational intelligence + operational coordination. **ALL FIVE Layer 1 communication primitives canonicalized at canon-faithful depth.** Phase W-4b Layer 1 implementation arcs (Calendar + SMS + Phone + Messaging) all open with locked canon foundation. Phase W-4b sequence step 6 (Communications layer service composition across all five primitives) opens with all five communication primitives' canon foundation locked.

**Coordination Focus arc dependency tracked symmetrically across all five communication primitives** — single Coordination Focus arc unblocks Email + Calendar + SMS + Phone + Messaging integration when concrete signals warrant.

**Wilbert demo concrete scenario locked at canon level**: five primitives + one operational state demonstration of cumulative architectural elegance, structurally inaccessible to fragmented competitor stacks (Slack + Salesforce + Calendly + Twilio + Gong).

**Total canon work post-Session-6**: Sessions 1 + 1.5 + 2 + 3 + 4 + 5 + 6 ≈ 25-29h dedicated canon work. Substantial; bounded; architecturally justified per §3.26.7.5 ("MVP cuts compound as debt"). Five communication primitives canonicalized at canon-faithful depth before any implementation-time canon-vs-implementation drift surfaces.

**Canon doc updates this session**: BRIDGEABLE_MASTER §3.26.19 NEW (20 subsections); DESIGN_LANGUAGE §14.13 NEW (6 sub-sections including cross-references); AESTHETIC_ARC Session 6 entry; CLAUDE.md §14 entry. **No code shipped.**

---

## Session 5 — Phone Primitive Canon (canon locked, 2026-05-13)

Pure architectural canon session matching Sessions 1 + 1.5 + 2 + 3 + 4 discipline pattern. NO CODE this session. Phone primitive canonicalized as **fourth and final Layer 1 communication primitive** completing operational substrate (after Email §3.26.15 + Calendar §3.26.16 + SMS §3.26.17); existing Call Intelligence implementation foundation (April 2026 — RingCentral + Deepgram Nova-2 + Claude extraction + after_call_service.py orchestrator + CallOverlay UI) explicitly preserved as canonical Phone primitive architectural substrate; integrate-now-make-native-later framework reaffirmed with RingCentral canonical primary + Twilio Voice deferred alternate; **communications + scheduling + reach + intelligence = operations** strategic positioning locked at canon level completing operational substrate four-part composition; multi-framework compliance discipline at primitive level distinct from prior primitives (state-by-state two-party consent + recording disclosure + HIPAA + FCC + CFPB); multi-dimensional cost discipline at primitive level (per-call + per-minute + per-transcription + per-intelligence-prompt) at higher magnitudes than SMS; real-time + asynchronous duality canonical at architectural model (Phone-distinctive distinguishing characteristic); call intelligence as primary value framework canonical (Phone-distinctive operational surface); voice input as platform substrate canonical (Phone primitive ships voice infrastructure consumed by other primitives); 27 canon subsections in BRIDGEABLE_MASTER + 8 visual canon subsections in DESIGN_LANGUAGE locking Phone primitive against settled foundation. Total ~3.5 hours conversational/architectural work; canon doc batch write at session close.

### Phase A — Entity Model + Provider Abstraction + Call Intelligence + Compliance + Cost (10 subsections)

§3.26.18 introduced as comprehensive Phone Primitive section (27 subsections — heaviest of four communication primitives proportional to Phone architectural complexity). Phone established as **fourth and final concrete Communications layer primitive** per Phase W-4b sequence step 4 (after Email Step 1 + Calendar Step 2 + SMS Step 3; sequencing per §3.26.6.4). Phase A locks the entity model + storage architecture + provider abstraction + call intelligence pipeline + compliance discipline + cost discipline; Phase B locks surfacing + composition + voice-mediated workflows + strategic vision integration; Phase C locks Focus + Workshop + Intelligence + Visual canon integration.

**Architectural framing — Phone as four-axis-distinct primitive** (§3.26.18): Phone is **architecturally heavier than SMS** on substrate (calls are real-time interactions PLUS asynchronous derived artifacts — recording + transcript + intelligence), **architecturally heavier than Calendar** on derived-artifact decomposition (call → recording → transcript → intelligence-artifact canonical decomposition), **architecturally heavier than Email** on intelligence-as-primary-value (call recordings are operationally meaningless without transcription + extraction + summarization), and **architecturally heavier on compliance complexity** (state-by-state two-party consent jurisdictional variability + HIPAA recording mode + FCC outbound discipline + CFPB collections discipline — multi-framework compliance distinct from Email's CAN-SPAM and SMS's TCPA + 10DLC).

**Real-time + asynchronous duality canonical at architectural model**: calls are simultaneously real-time interactions (operator-on-call live experience) AND asynchronous artifacts (post-call recording + transcript + intelligence + state-change action items). The Phone primitive captures both dimensions canonically — Call entity owns real-time identity + outcome; CallRecording + CallTranscript + CallIntelligenceArtifact entities own asynchronous derived content per §3.26.18.2 entity decomposition discipline.

**Call intelligence as primary value framework**: raw audio recording is operationally low-value at scale (operators don't relisten to hours of calls); transcription + intent extraction + summarization + entity resolution + action item extraction is the canonical operational surface. Call Intelligence pipeline per §3.26.18.6 is the primary value generator — recording + raw audio is preserved as compliance artifact + audit substrate, NOT as primary operator-facing content. This framing distinguishes Phone primitive from Email + Calendar + SMS.

**Existing Call Intelligence implementation as foundation**: the platform shipped Call Intelligence (April 2026 — RingCentral OAuth + extension-to-user mapping + Deepgram Nova-2 transcription + Claude extraction + after-call orchestrator with 10s delay + CallOverlay UI with 3 states + voicemail handling) as a working feature with substantial customer tuning. Phone primitive canon does NOT discard this implementation — it formalizes it as canonical Phone-primitive architectural model + extends with multi-provider abstraction + canonical entity decomposition + cross-primitive integration. The existing `ringcentral_connections` + `ringcentral_extension_mappings` + `ringcentral_call_log` + `ringcentral_call_extractions` tables map to canonical Phone primitive entity model — implementation arc executes provider-rename + entity-decomposition + extension migrations, not greenfield rebuild.

**Native-vs-integration deliberation reaffirmed verbatim** (§3.26.18.1) preserving architectural rationale: native phone implementation from day one would require SIP trunking carrier integration + per-carrier compliance attestation + RTP media handling + media server infrastructure + STIR/SHAKEN attestation infrastructure + DTMF + supplementary services + voicemail platform + 911 emergency services compliance + international roaming carrier negotiations + carrier-direct billing reconciliation + per-carrier number portability mechanics. The integrate-now-make-native-later commitment is canonical — Bridgeable's Phone primitive owns its **architectural model** while delegating **transport** to provider abstraction (RingCentral canonical primary + Twilio Voice alternate per §3.26.18.3 + concrete signal). Future native carrier integration deferred indefinitely.

**Entity model canonical** (§3.26.18.2): six core entities — two primary (Call + CallParticipant) + three derived-artifact (CallRecording + CallTranscript + CallIntelligenceArtifact) + supporting (PhoneAccount + PhoneAccountAccess junction). Plus cross-tenant pairing via `cross_tenant_call_pairing`. CallTranscript carries `superseded_by_transcript_id` FK self-reference (Phase A refinement integrated) for explicit version chain audit history when re-transcription occurs.

**Provider abstraction canonical** (§3.26.18.3): RingCentral canonical primary at September scope. Twilio Voice deferred alternate per §3.26.18.23 with three documented signals: (1) tenant has existing Twilio Voice contract + operational call volume; (2) tenant operational pattern requires Twilio-specific feature like programmable voice routing or TwiML IVR; (3) sustained tenant signal indicating RingCentral provisioning friction. Native PBX integration + international providers deferred indefinitely.

**Real-time architectural canon** (§3.26.18.4 + §3.26.18.6): Deepgram real-time API (WebSocket connection per active call; partial transcripts at ~250ms cadence); WebSocket/SSE channel for live intelligence delivery to CallOverlay; provider-side real-time webhook subscriptions; reconciliation discipline at call end (partial transcripts → canonical CallTranscript via batch API re-processing; intelligence pipeline re-execution; superseded versions audit-preserved via `superseded_by_artifact_id`).

**Multi-framework compliance discipline canonical at primitive level** (§3.26.18.9): state-by-state two-party consent enforcement (13-state jurisdictional config) + recording disclosure mechanics (TTS prompt + 5-second delay) + HIPAA-compliant recording mode + FCC TCRA compliance + CFPB Regulation F discipline + cross-primitive opt-out coordination registry. Cross-primitive opt-out coordination canonical: caller-side opt-out detected via call intelligence pipeline propagates to all communication primitives — email + SMS + calendar + phone outbound suspension via cross-primitive opt-out registry. Pattern inherited by future Layer 1 communication primitives.

**Multi-dimensional cost discipline canonical at primitive level** (§3.26.18.10): per-call + per-minute + per-transcription + per-intelligence-prompt cost dimensions. Total per-call cost magnitude $0.20-0.50 for typical 30-minute call with full intelligence pipeline. Workshop cost-policy templates apply across cost dimensions. Concrete competitor comparison canonicalized: standalone call intelligence platforms (Gong + Chorus + Fireflies + Otter) charge per-seat licensing ($60-150/seat/month); Bridgeable's Phone-as-platform-primitive framing eliminates context-switch + per-primitive cost discipline replaces per-seat licensing.

### Phase A — Q1-Q12 deliberations + 5 refinements integrated

Twelve architectural questions surfaced + resolved with five refinements integrated:

- **Q1 RingCentral canonical primary** confirmed; Twilio Voice deferred with three documented signals canonicalized
- **Q2 6-entity decomposition** confirmed (Call + CallParticipant + CallRecording + CallTranscript + CallIntelligenceArtifact + PhoneAccount + PhoneAccountAccess); CallTranscript `superseded_by_transcript_id` FK self-reference added (Phase A refinement integrated)
- **Q3 3-value `recording_default` enum** confirmed (always_record / operator_toggle / disabled); operator-disclosure-on-call-start TTS prompt + 5-second delay canonical
- **Q4 Deepgram Nova-2 canonical primary** confirmed; Whisper alternate deferred with concrete signal; real-time transcription canonical at September scope
- **Q5 7 managed prompts canonical** confirmed (call_summarize Sonnet + call_intent_classify Haiku + call_action_items_extract Sonnet + call_entity_resolve Haiku + call_sentiment_classify Haiku + compliance_review Haiku + voicemail_summarize Haiku); `phone.call_quality_review` (Sonnet) deferred with concrete signal documented (tenant operational pattern signals QA program need)
- **Q6 differential retention** confirmed (Call indefinite + CallRecording 7-year + CallTranscript 10-year + CallIntelligenceArtifact indefinite/superseded-90-day); legal-hold cascades; release of hold cascades release
- **Q7 comprehensive multi-framework compliance** confirmed with strengthening; cross-primitive opt-out coordination canonical added (Phase A refinement integrated)
- **Q8 multi-dimensional cost discipline** confirmed with strengthening; competitor comparison closing prose canonicalized (Gong + Chorus + Fireflies + Otter; Phase A refinement integrated)
- **Q9 voicemail as canonical sub-state of Call** confirmed (`outcome="voicemail"` with associated CallRecording + CallTranscript + CallIntelligenceArtifact); voicemail-as-distinct-entity rejected
- **Q10 simple per-account routing** confirmed canonical at September; complex IVR-equivalent routing tree deferred with three documented signals
- **Q11 real-time call surface** confirmed canonical at September scope; real-time architectural canon integrated (Phase A refinement integrated)
- **Q12 cross-tenant call bilateral consent** confirmed (parallel to §3.26.16.6 calendar bilateral consent canon); per-tenant audit trails + per-tenant intelligence pipeline + per-tenant recording copies canonical

### Phase B — Surfacing + Composition + Voice-Mediated Workflows + Strategic Vision Integration (13 subsections)

§3.26.18.11-§3.26.18.23. Three-shape surface decomposition: unified call surface (call-workspace-shape at `/calls` distinct from Space architecture per §3.26.18.11) + Pulse Phone rendering across multiple layers (Communications + Operational layers per §3.26.18.12 hybrid contribution pattern) + real-time call surface canonical at September per §3.26.18.22.

**Hybrid layer contribution pattern canonical** (§3.26.18.12, parallel to §3.26.16.10 calendar + §3.26.17.11 SMS hybrid contribution): Phone contributes to MULTIPLE Pulse layers — Communications layer (customer-facing call signals) + Operational layer (operational-team-facing call signals). Pattern continuity: Email + Calendar + SMS + Phone all use hybrid layer contribution per §3.26.9 canonical pattern. Future Session 6 in-platform messaging primitive inherits same pattern.

**Customer-facing vs operational-team-facing conversation discipline** (§3.26.18.21): single primitive with `conversation_orientation` parameter (`customer_facing` | `operational_team_facing`); orientation is composition parameter, not primitive split.

**Five canonical action_types** (§3.26.18.20): `outbound_call_attempt_acknowledgment` + `inbound_call_action_item_review` + `compliance_review_required` + `voicemail_follow_up` + `cross_tenant_operational_acknowledgment`. Phone primitive's intelligence-mediated coupling is structurally different — when customer accepts quote on phone call, intelligence extracts "Customer accepted quote" as action item via `inbound_call_action_item_review` substrate; operator reviews + commits state change. Routes through unified action item substrate, not parallel direct-confirmation action_types matching email/calendar/SMS direct-confirmation patterns. All reuse `platform_action_tokens` substrate per Calendar Q13 substrate consolidation direction.

**Total cross-primitive action_types canonical at canon level (post-Session-5)**: Email 1 + Calendar 5 + SMS 5 + Phone 5 = **16 action_types** all reusing `platform_action_tokens` substrate.

**Communications + scheduling + reach + intelligence = operations strategic framing locked** (§3.26.18.18, four-part operational substrate completion): Phone primitive's strategic role completes operational substrate four-part composition. Email primitive ships interpersonal coordination; Calendar primitive ships temporal coordination; SMS primitive ships operational reach; Phone primitive ships **operational intelligence** — call recordings + transcripts + intelligence-derived artifacts feed operational state directly via call-intelligence-as-primary-value framework. Action items extracted from call intelligence propagate state changes; operational state changes generate outbound call drafts. Phone is not a separate communication island; Phone is the intelligence-bearing communication primitive that completes operational substrate four-part composition.

**Three architectural justifications preserved at canon level for "Communications + scheduling + reach + intelligence = operations" framing**:
1. Phone-distinctive among Layer 1 communication primitives — intelligence-as-primary-value per §3.26.18.1 is genuinely Phone-specific architectural shape
2. Voice modality alternative is platform substrate, not Phone-specific dimension — voice input canonicalized at §3.26.18.16 as platform substrate consumed by Email + NL Creation + Command Bar + future Bridgeable Assist + future Bridgeable Vision
3. Real-time + asynchronous duality alternative is architectural model concern, not operator-experiential concern

**Voice-mediated workflows canon at September scope** (§3.26.18.16): Phone primitive is the natural primitive for voice input infrastructure. Voice input infrastructure — Web Speech API + Deepgram fallback + push-to-talk affordance (preferred default; operator agency discipline; auto-listen anti-pattern explicitly rejected) + audio waveform visualization. Voice-mediated Command Bar (existing SmartPlantCommandBar pattern). Voice-mediated entity reference (NL Creation overlay + cross-primitive composition). Hands-free operational use cases canonical at September (driver console + production floor terminals + dispatcher kiosks + FH director arrangement scribe). Voice input is platform substrate per architectural-elegance discipline.

**Voice input cost attribution discipline canonical** (§3.26.18.16, Phase B refinement integrated): voice input transcription cost attribution canonical: voice input consumed by primitive X attributes transcription cost to primitive X's cost discipline. Email composition voice input → Email primitive cost. Phone primitive voice input → Phone primitive cost. Command Bar voice input → summoning primitive cost (resolved at action commit time). NL Creation overlay voice input → target entity primitive cost. Cross-primitive voice input substrate preserves per-primitive cost attribution clarity; tenant cost-policy templates apply per primitive consuming voice substrate.

### Phase B — Q13-Q19 deliberations + 1 refinement integrated

Seven architectural questions surfaced + resolved with one refinement integrated:

- **Q13 separate /calls route** confirmed (workspace-shape distinct from Spaces architecture; split-pane call detail composition canonical — co-presentation discipline preserves Phone primitive's structural reality)
- **Q14 hybrid layer contribution** confirmed (Communications + Operational layers split parallel to Calendar Q9 + SMS Q11)
- **Q15 "Communications + scheduling + reach + intelligence = operations"** confirmed canonical strategic framing; three architectural justifications preserved at canon level; concrete competitor comparison (Gong + Chorus + Fireflies + Otter) preserved at canon level
- **Q16 voice-mediated workflows canonical at September** confirmed; voice input cost attribution discipline added (Phase B refinement integrated)
- **Q17 5 canonical action_types** confirmed; parallel direct-confirmation action_types matching email/calendar/SMS rejected (Phone's intelligence-mediated coupling is structurally different)
- **Q18 single primitive with conversation_orientation parameter** confirmed (customer_facing | operational_team_facing per §3.26.18.21)
- **Q19 real-time call infrastructure canonical at full depth** confirmed (live transcript ~250ms cadence + live intelligence WebSocket/SSE + reconciliation post-call all canonical at September)

### Phase C — Focus + Workshop + Intelligence Integration + Visual Canon (4 BRIDGEABLE_MASTER subsections + 8 DESIGN_LANGUAGE subsections)

§3.26.18.24-§3.26.18.27 + §14.12. Phone-triggered Decision Focus integration ships at canon-faithful depth — six canonical triage queue config types at September scope (`phone_compliance_review_triage` + `phone_action_item_review_triage` + `phone_outbound_attempt_review_triage` + `phone_voicemail_follow_up_triage` + `phone_cross_tenant_acknowledgment_triage` + `phone_cost_warning_triage` Phone-distinctive among Layer 1 communication primitives because Phone's multi-dimensional cost magnitudes make cost warning canonically valuable). Phase 5 triage primitive substrate enables implementation when Phase W-4b Phone Step ships triage integration (no Coordination-Focus-arc dependency).

**Phone-mediated Coordination Focus deferred** — five concrete signals (single-call deliberation exceeds resolution scope; multi-party call coordination across cross-tenant boundaries; long-running call series; action item dependencies across calls; compliance-escalated call series); deferral is **symmetric across email-mediated AND calendar-mediated AND SMS-mediated AND phone-mediated Coordination Focus integration**; single Coordination Focus arc unblocks all four communication primitives' integration when concrete signals warrant.

**Phone Intelligence at seven-prompt September scope**: `phone.call_summarize` (Sonnet) + `phone.call_intent_classify` (Haiku) + `phone.call_action_items_extract` (Sonnet) + `phone.call_entity_resolve` (Haiku) + `phone.call_sentiment_classify` (Haiku) + `phone.compliance_review` (Haiku) + `phone.voicemail_summarize` (Haiku). Real-time prompt firing discipline canonical (intent first-pass at 30s; action items first-pass at 60s; sentiment ~500ms; compliance ~1s); post-call reconciliation re-execution against canonical transcript with `superseded_by_artifact_id` audit chain. Operator agency preserved via canonical anti-patterns.

**Phone Workshop at seven-template-type granularity** (Q21 confirmation): seven distinct Workshop template types — `call_template` + `recording_rule_template` + `cost_policy_template` + `call_routing_rule_template` + `jurisdictional_config_template` + `intelligence_prompt_policy_template` + `provider_provisioning_template`. Meta-pattern continuity preserved — Workshop template granularity is determined by structural-overlap-of-composition-shapes per §3.26.16.24 calendar precedent. Phone's seven template shapes are structurally different (call script ≠ recording rule ≠ cost policy ≠ routing rule ≠ jurisdictional config ≠ intelligence-prompt policy ≠ provider provisioning) → per-template-type appropriate.

**Total cross-primitive Workshop template types canonical at canon level (post-Session-5)**: Email 1 + Calendar 3 + SMS 5 + Phone 7 = **16 Workshop template types**. Meta-pattern locks across all four communication primitives. Future Session 6 in-platform messaging primitive applies same heuristic.

**Phone Visual Canon extends §14 Communications Visual System** (Q20): §14.12 ships eight subsections matching §14.10 Calendar precedent + §14.9 Email precedent + §14.11 SMS precedent + Phone-distinctive subsections — §14.12.1 Phone call list (call-workspace-shape distinction from Space architecture) + §14.12.2 Call detail rendering split-pane composition (transcript pane + intelligence pane) + §14.12.3 Real-time call surface (Phone-distinctive; live transcript + live intelligence + minimized call pill + reconciliation discipline visualization) + §14.12.4 Call composition surface (operator-initiated outbound modal + Workshop-template-generated outbound modal + cost breakdown chrome) + §14.12.5 Cross-tenant call chrome (cross-tenant indicator + bilateral consent state + per-tenant audit + cross-tenant masking) + §14.12.6 Operational-action affordance chrome (5 action_types) + §14.12.7 Voice-mediated affordance chrome (Phone-distinctive; push-to-talk + cross-primitive consumption + audio waveform) + §14.12.8 Cross-references.

### Phase C — Q20-Q22 deliberations

Three architectural questions surfaced + resolved:

- **Q20 §14.12 extends §14 Communications Visual System** confirmed (eight subsections matching §14.10 Calendar precedent + §14.9 Email precedent + §14.11 SMS precedent; Phone-distinctive §14.12.3 Real-time call surface + §14.12.7 Voice-mediated affordance chrome that prior primitives didn't need)
- **Q21 per-template-type with seven template types** confirmed; jurisdictional_config_template NOT consolidated into recording_rule_template — structural difference (platform-level vs tenant-level + annual vs operational + jurisdictional table vs disclosure script TTS) justifies distinct types; meta-pattern continuity preserved
- **Q22 §14.12.3 Real-time call surface visual canon** confirmed canonical at September scope (cross-references Q11 + Q19 confirmed; canonical visual + interaction model preserves operator UX continuity)

### Phase A + B + C refinements integrated

- 5 Phase A refinements: CallTranscript `superseded_by_transcript_id` FK self-reference (Q2); Twilio Voice three documented signals + `phone.call_quality_review` deferral signal + IVR-equivalent routing three documented signals (Q23 deferral catalog); real-time architectural canon Deepgram real-time API + WebSocket/SSE + reconciliation discipline (Q11/§3.26.18.4 + §3.26.18.6); cross-primitive opt-out coordination canon (Q7/§3.26.18.9); competitor comparison closing prose Gong + Chorus + Fireflies + Otter (Q8/§3.26.18.10)
- 1 Phase B refinement: voice input cost attribution discipline canonical (Q16/§3.26.18.16) — per-primitive cost attribution preserves clarity across cross-primitive voice input substrate consumption
- 22 architectural questions Q1-Q22 deliberated and resolved across three phases

**Architectural model post-Session-5**: 5 primitives + fractal Pulse + universal Command Bar + Workshop primitive + comprehensive Email + Calendar + SMS + Phone primitive canon. Email + Calendar + SMS + Phone = four communication primitives canonicalized at canon-faithful depth. Together they form the **operational substrate four-part composition** — interpersonal coordination + temporal coordination + operational reach + operational intelligence. Phase W-4b Layer 1 implementation arcs (Calendar + SMS + Phone) all open with locked canon foundation. In-platform messaging primitive (Session 6 candidate) sequences forward — tightest scope of four communication primitives.

**Coordination Focus arc dependency tracked symmetrically across Email + Calendar + SMS + Phone** — single Coordination Focus arc unblocks all four communication primitives' integration when concrete signals warrant.

**Voice input as platform substrate canonical** — Phone primitive ships voice input infrastructure; Email + Calendar + SMS + NL Creation overlay + Command Bar + future Bridgeable Assist + future Bridgeable Vision all consume voice input via shared infrastructure. Per-primitive cost attribution preserves clarity.

**Existing Call Intelligence implementation foundation explicitly preserved** — implementation arc executes provider-rename + entity-decomposition + extension migrations against existing tables, NOT greenfield rebuild. Months of customer tuning preserved (extraction accuracy, draft order quality, voicemail handling).

**Total canon work post-Session-5**: Sessions 1 + 1.5 + 2 + 3 + 4 + 5 ≈ 22-26h dedicated canon work. Substantial; bounded; architecturally justified per §3.26.7.5 ("MVP cuts compound as debt"). Four communication primitives canonicalized at canon-faithful depth before any implementation-time canon-vs-implementation drift surfaces.

**Canon doc updates this session**: BRIDGEABLE_MASTER §3.26.18 NEW (27 subsections); DESIGN_LANGUAGE §14.12 NEW (8 sub-sections including cross-references); AESTHETIC_ARC Session 5 entry; CLAUDE.md §14 entry. **No code shipped.**

---

## Session 4 — SMS Primitive Canon (canon locked, 2026-05-12)

Pure architectural canon session matching Sessions 1 + 1.5 + 2 + 3 discipline pattern. NO CODE this session. SMS primitive canonicalized as third concrete Phase W-4b Layer 1 communication primitive (after Email §3.26.15 + Calendar §3.26.16); integrate-now-make-native-later framework reaffirmed with Twilio as canonical provider + native CTIA participation deferred indefinitely; **communications + scheduling + reach = operations** strategic positioning locked at canon level completing operational substrate trifecta (Email = interpersonal coordination + Calendar = temporal coordination + SMS = operational reach); TCPA + 10DLC compliance discipline at primitive level distinct from Email/Calendar precedent; per-message cost discipline at primitive level distinct from any prior primitive; outbound-weighted operational-coupling pattern (state-changes-generate-SMS dominant per §3.26.17.18) distinct from Email's inbound-weighted pattern; 24 canon subsections in BRIDGEABLE_MASTER + 6 visual canon subsections in DESIGN_LANGUAGE locking SMS primitive against settled foundation. Total ~3.5 hours conversational/architectural work; canon doc batch write at session close.

### Phase A — Entity Model + Storage + Compliance + Cost (9 subsections)

§3.26.17 introduced as comprehensive SMS Primitive section (24 subsections matching §3.26.15 Email + §3.26.16 Calendar primitive depth). SMS established as **third concrete Communications layer primitive** per Phase W-4b sequence step 3 (after Email Step 1 + Calendar Step 2; sequencing per §3.26.6.4). Phase A locks the entity model + storage architecture + provider abstraction + compliance discipline + cost discipline; Phase B locks surfacing + composition + strategic vision integration; Phase C locks Focus + Workshop + Intelligence integration + Visual canon.

**Architectural framing — SMS as three-axis-distinct primitive** (§3.26.17): SMS is **architecturally lighter than Email** on substrate (no HTML rendering; no threading complexity; no MIME parsing; tighter 1600-char body) and **architecturally narrower than Calendar** on provider abstraction (Twilio dominant US provider; Email shipped 4 providers, Calendar shipped 4, SMS bounded narrower) but **architecturally heavier on compliance discipline** (TCPA + 10DLC at primitive level distinct from Email's CAN-SPAM and Calendar's iCalendar standard) and **architecturally heavier on cost discipline** (per-message cost ~$0.0075 outbound per §3.26.17.9 distinct from Email's bulk send economics). Three-axis-distinct shape shapes canon — entity model is straightforward; compliance + cost integration is comprehensive; provider abstraction is narrower but high-fidelity Twilio integration.

**Native-vs-integration deliberation reaffirmed verbatim** (§3.26.17.1) preserving architectural rationale: native SMS implementation from day one would require SMPP carrier integration + direct CTIA registry participation + per-carrier 10DLC compliance attestation + per-carrier short-code or long-code provisioning + carrier-direct routing infrastructure + per-carrier deliverability monitoring + international roaming carrier negotiations (deferred indefinitely — US-only at canonical scope) + carrier-direct billing reconciliation. The integrate-now-make-native-later commitment is canonical — Bridgeable's SMS primitive owns its **architectural model** (entity model + UI surfaces + Intelligence integration + operational coupling + compliance discipline + cost discipline) while delegating **transport** to provider abstraction (Twilio canonical; Bandwidth + alternate providers deferred per §3.26.17.4 + concrete signal). Future native CTIA participation deferred indefinitely — Twilio's CTIA participation + 10DLC infrastructure + carrier relationships are operationally irreplaceable at canonical scale.

**Entity model canonical** (§3.26.17.2): five core entities form the SMS primitive substrate — three primary (Conversation + Message + Participant) + two supporting (Account + AccountAccess junction) + optional MMS attachment entity. Entity decomposition matches §3.26.15.2 email primitive entity model + §3.26.16.2 calendar primitive entity model — same M:N junction pattern; same auto-resolution + manual + multi-linkage discipline; same independent-per-tenant-cross-tenant-copies discipline. Plus cross-tenant pairing via `cross_tenant_sms_pairing` parallel to email + calendar precedent.

**Provider abstraction canonical** (§3.26.17.4): Twilio dominant canonical scope. Bandwidth + Telnyx + alternate providers deferred per §3.26.17.4 + concrete signal documented (sustained tenant signal indicating provider lock-in friction). Provider abstraction narrower than Email (4 providers) + Calendar (4 providers) — SMS provider concentration reflects US carrier ecosystem reality + 10DLC infrastructure complexity.

**TCPA + 10DLC compliance discipline at primitive level** (§3.26.17.8): per-conversation opt-in tracking via 4-value enum (`opted_in` / `opted_out` / `opt_in_pending` / `unknown`) + 5-value method enum (`provider_imported` / `customer_initiated_inbound` / `manual_attestation` / `web_form` / `paper_form`) + indefinite audit log + STOP/HELP carrier-side handling + revocation discipline. Compliance is operational protection, not bureaucratic burden — runaway-compliance scenarios structurally impossible.

**10DLC hybrid registration model** (§3.26.17.3): platform-owned brand + tenant-owned campaign. Bridgeable's TCR brand registration is shared across all platform tenants (`tcr_brand_id` populated at platform level); per-tenant 10DLC campaign registration is tenant-owned (`tcr_campaign_id` populated when tenant has registered campaign). Tenant operators do NOT need to register their own 10DLC brand — Bridgeable's platform brand registration provides operational coverage. Bridgeable provides Workshop-authored campaign-registration template per §3.26.17.24.

**Per-message cost discipline at primitive level** (§3.26.17.9): per-tenant + per-account + per-conversation cost tracking + Workshop-authored cost policies + pre-send budget cap enforcement. Per-message cost ~$0.0075 outbound (Twilio canonical). Pre-send budget cap enforcement structurally prevents runaway-cost scenarios — tenant operators set budget caps per period; Bridgeable enforces at compose-preview surface per §3.26.17.5 pre-send validation pipeline.

### Phase A — Q1-Q9 deliberations + 4 refinements integrated

Nine architectural questions surfaced + resolved with four refinements integrated:

- **Q1 hybrid 10DLC registration** confirmed (platform-owned brand + tenant-owned campaign per §3.26.17.3); short-code signal threshold canonicalized — sustained >50,000 outbound SMS/month per tenant + tenant willingness to fund short-code lease (~$1000/month) bounded measurable trigger prevents re-litigation
- **Q2 Twilio canonical** confirmed (single dominant provider); concrete TCR lead-time discipline canonicalized — 10DLC campaign registration takes 1-3 weeks; tenant onboarding flow accommodates lead-time via Workshop campaign-registration template + status tracking
- **Q3 from/to + 30-day window canonical conversation reconstruction** confirmed (Bridgeable mirrors carrier-side threading verbatim per §3.26.17.4)
- **Q4 comprehensive TCPA discipline** confirmed (4-value enum + 5-value method enum + indefinite audit log + STOP/HELP handling + revocation discipline per §3.26.17.8)
- **Q5 comprehensive cost discipline** confirmed (per-message cost tracking + budget caps + Workshop cost policies per §3.26.17.9); concrete competitor comparison prose canonicalized — Twilio Studio + MessageBird Flow Builder + Zapier SMS workflows treat SMS as standalone tool requiring operator context-switch; Bridgeable's SMS-as-operational-primitive eliminates context-switch
- **Q6 hybrid 10DLC registration** confirmed (platform brand + tenant campaign per §3.26.17.3)
- **Q7 industry-canonical opt-out keywords** confirmed (STOP / UNSUBSCRIBE / END / QUIT / CANCEL all trigger opt-out per carrier-side handling); custom emergency keywords deferred per §3.26.17.20 — tenant-specific custom keywords canonicalize via Workshop opt-in-flow templates per §3.26.17.24
- **Q8 MMS bounded with image size discipline** confirmed (single image per message per §3.26.17.4 + §3.26.17.5); image size discipline canonicalized — 1MB Twilio canonical max; provider config max_size_kb per-account override; multi-attachment MMS deferred per §3.26.17.20
- **Q9 7-year retention default** confirmed (hybrid retention discipline per §3.26.17.6; tenant-configurable to 90-day minimum; hard-delete after window)

### Phase B — Surfacing + Composition + Strategic Vision Integration (11 subsections)

§3.26.17.10-§3.26.17.20. Three-shape surface decomposition: unified messaging surface (SMS-workspace-shape at `/sms` distinct from Space architecture per §3.26.17.10 — clarification prevents canon-vs-implementation drift) + Pulse SMS rendering across multiple layers (Communications + Operational layers per §3.26.17.11 hybrid contribution pattern) + Workshop-template send modal per §3.26.17.14.

**Hybrid layer contribution pattern canonical** (§3.26.17.11, parallel to §3.26.16.10 calendar precedent): SMS contributes to MULTIPLE Pulse layers based on signal type — Communications layer ("who needs me?" interpersonal SMS signals) + Operational layer ("what's my work right now?" operational-team SMS signals). NOT a dedicated SMS layer. Pattern locks for future Layer 1 communication primitives — phone call-incoming-vs-alarm signals, messaging direct-mention-vs-system-event signals all decompose along same cognitive-question split. Pattern continuity: Email + Calendar + SMS all use hybrid layer contribution.

**Customer-facing vs operational-team-facing conversation discipline** (§3.26.17.19): single primitive with `conversation_orientation` parameter (`customer_facing` | `operational_team_facing`). Single primitive — both orientations share entity model + provider + compliance + cost discipline; orientation is composition parameter, not primitive split. Drives default Pulse layer assignment + default routing on outbound + default visual chrome surfacing per §14.11 visual canon.

**Five canonical action_types** (§3.26.17.18): `customer_confirmation` + `driver_check_in_confirmation` + `delivery_arrival_acknowledgment` + `service_day_acknowledgment` + `cross_tenant_operational_acknowledgment`. **Two action patterns canonical**: keyword-reply (strict matching: case-insensitive + trailing whitespace tolerant only) + magic-link (single-action commit; tokens reuse `platform_action_tokens` substrate per §3.26.16.17 substrate consolidation). Strict keyword-reply matching at canonical scope preserves operator agency per §3.26.14.14.5 — fuzzy keyword matching via Bridgeable Intelligence (`sms.intent_classify` per §3.26.17.23) deferred per §3.26.7.5 with concrete signal documented (operator pattern signals fuzzy matching value AND operator agency discipline preserved through review-before-commit path).

**Communications + scheduling + reach = operations strategic framing locked** (§3.26.17.16, trifecta-completion): SMS primitive's strategic role completes operational substrate trifecta. Email primitive (§3.26.15) ships interpersonal coordination; Calendar primitive (§3.26.16) ships temporal coordination; SMS primitive (§3.26.17) ships **operational reach** — Layer 1 primitive that bypasses surface-navigation friction and reaches operators + customers directly in their pocket. **Concrete competitor comparison canonicalized**: Twilio Studio + MessageBird Flow Builder + Zapier SMS workflows + alternate SMS workflow tools treat SMS as standalone workflow tool — operators leave operational context (CRM / ERP / scheduling tool) to manage SMS workflows in separate tool. State coherence between SMS workflow + operational state is operator-maintained manually. Bridgeable's SMS-as-operational-reach-primitive framing eliminates this context-switch by treating SMS as platform primitive — operational state generates SMS automatically per §3.26.17.18; SMS responses propagate state changes back per §3.26.17.18 inverse coupling; operators never leave operational context to manage SMS. Sharpens September Wilbert demo positioning at canon level.

**State-changes-generate-SMS canonicalized at depth** (§3.26.17.18): outbound-weighted operational-coupling more pervasive than Email's inverse coupling. Canonical state-change → SMS mappings for Delivery scheduling notifications, Driver check-in confirmations, Delivery arrival acknowledgments, Service day acknowledgments, Cross-tenant operational acknowledgments. Drafted-not-auto-sent discipline preserves operator agency per §3.26.14.14.5. Inverse: keyword-reply or magic-link propagates state changes — bidirectional binding canonical.

### Phase B — Q10-Q14 deliberations + 2 refinements integrated

Five architectural questions surfaced + resolved with two refinements integrated:

- **Q10 separate /sms route** confirmed (SMS-workspace-shape distinct from Space architecture per §3.26.17.10; parallel to §3.26.16.9 calendar-workspace-shape distinction)
- **Q11 hybrid layer contribution pattern** confirmed (Communications + Operational layers split per §3.26.17.11; pattern lock for future Layer 1 primitives)
- **Q12 5 action_types + 2 patterns with strict keyword-reply matching** confirmed (per §3.26.17.18); strict matching refinement canonicalized — case-insensitive + trailing whitespace tolerant only; fuzzy matching deferred per §3.26.7.5 with concrete signal documented
- **Q13 single primitive with conversation_orientation parameter** confirmed (customer_facing | operational_team_facing per §3.26.17.19)
- **Q14 SMS-as-operational-reach trifecta-completion framing** confirmed (per §3.26.17.16); trifecta-completion strategic prose canonicalized — Email + Calendar + SMS together ARE the operational substrate of the platform; concrete competitor comparison sharpens September Wilbert demo positioning

### Phase C — Focus + Workshop + Intelligence Integration + Visual Canon (4 subsections + 6 visual canon subsections)

§3.26.17.21-§3.26.17.24 + §14.11. SMS-triggered Decision Focus integration ships at canon-faithful depth — five canonical triage queue config types at September scope (`sms_compliance_review_triage` + `sms_response_review_triage` + `sms_budget_warning_triage` + `sms_action_review_triage` + `sms_opt_in_pending_triage`); Phase 5 triage primitive substrate enables implementation when Phase W-4b SMS Step ships triage integration (no Coordination-Focus-arc dependency).

**SMS-mediated Coordination Focus deferred** — five concrete signals from Step 5b investigation; deferral is **symmetric across email-mediated AND calendar-mediated AND SMS-mediated Coordination Focus integration**; single Coordination Focus arc unblocks all three communication primitives per §3.26.17.22.

**SMS Intelligence at four-prompt September scope** (§3.26.17.23): `sms.intent_classify` (Haiku) + `sms.priority_classification` (Haiku) + `sms.compliance_review` (Haiku) + `sms.response_draft` (Sonnet). Operator agency preserved via canonical anti-patterns; suggestion telemetry privacy-preserving per §3.26.17.6 cross-tenant masking inheritance discipline.

**SMS Workshop at five-template-type granularity** (§3.26.17.24, Q15 confirmation): five distinct Workshop template types — `sms_template` + `sms_opt_in_flow_template` + `sms_campaign_registration_template` + `sms_cost_policy_template` + `sms_response_router_template`. **Meta-pattern continuity**: Workshop template granularity is determined by structural-overlap-of-composition-shapes per §3.26.16.24 calendar precedent. SMS's five template shapes are structurally different — opt-in flow (multi-step keyword conversation) ≠ campaign registration (TCR submission helper) ≠ cost policy (budget enforcement rules) ≠ response router (keyword-to-action mapping) ≠ message template (single-message render) → per-template-type appropriate. Future Workshop integrations (phone, messaging) apply same heuristic.

**SMS Visual Canon extends §14 Communications Visual System** (Q15): §14.11 ships six subsections matching §14.10 Calendar precedent + §14.9 Email precedent — §14.11.1 SMS conversation list (SMS-workspace-shape distinction from Space architecture) + §14.11.2 SMS conversation detail (chronological message sequence + Pattern 2 sub-cards distinct from Email's Pattern 2 cards) + §14.11.3 SMS composition surface (new conversation modal + inline reply + Workshop-template send modal) + §14.11.4 Cross-tenant SMS conversation chrome (cross-tenant indicator + bilateral consent state + MMS visual chrome + compliance audit chrome) + §14.11.5 Operational-action affordance chrome (keyword-reply + magic-link + MMS photo proof affordance) + §14.11.6 Cross-references.

### Phase C — Q15-Q16 deliberations

Two architectural questions surfaced + resolved:

- **Q15 §14.11 extends §14 Communications Visual System** confirmed (six subsections matching §14.10 Calendar precedent + §14.9 Email precedent)
- **Q16 per-template-type with five template types** confirmed (sms_template + sms_opt_in_flow_template + sms_campaign_registration_template + sms_cost_policy_template + sms_response_router_template per §3.26.17.24); meta-pattern continuity preserved (structural-overlap-of-composition-shapes determines per-template-type vs single-template-type per §3.26.16.24 calendar precedent)

### Phase A + B + C refinements integrated

- 4 Phase A refinements: short-code signal threshold (Q1); TCR lead-time discipline (Q2); concrete competitor comparison prose (Q5); image size discipline (Q8)
- 2 Phase B refinements: strict keyword-reply matching + fuzzy matching deferral (Q12); trifecta-completion strategic prose (Q14)
- 16 architectural questions Q1-Q16 deliberated and resolved across three phases

**Architectural model post-Session-4**: 5 primitives + fractal Pulse + universal Command Bar + Workshop primitive + comprehensive Email + Calendar + SMS primitive canon. Email + Calendar + SMS = three communication primitives canonicalized at canon-faithful depth. Together they form the **operational substrate trifecta** — interpersonal coordination + temporal coordination + operational reach. Phone (Session 5 candidate) + Messaging (Session 6 candidate) sequence forward as Phase W-4b implementation arc proceeds.

**Coordination Focus arc dependency tracked symmetrically across Email + Calendar + SMS** — single Coordination Focus arc unblocks all three communication primitives' integration when shipped.

**Total canon work post-Session-4**: Sessions 1 + 1.5 + 2 + 3 + 4 ≈ 19-22h dedicated canon work. Substantial; bounded; architecturally justified per §3.26.7.5 ("MVP cuts compound as debt"). Three communication primitives canonicalized at canon-faithful depth before any implementation-time canon-vs-implementation drift surfaces.

**Canon doc updates this session**: BRIDGEABLE_MASTER §3.26.17 NEW (24 subsections); DESIGN_LANGUAGE §14.11 NEW (6 sub-sections including cross-references); AESTHETIC_ARC Session 4 entry; CLAUDE.md §14 entry. **No code shipped.**
...
§3.26.15 introduced as comprehensive Email Primitive section (25 subsections). Email established as **first concrete Communications layer primitive** per Phase W-4b sequence step 1 (preceding Calendar + SMS + Phone + Messaging). Phase A locks the entity model + storage architecture + provider abstraction; Phase B locks surfacing + summoning + composition; Phase C locks Focus + Workshop + Intelligence integration.

**Native-vs-integration deliberation prose canonicalized verbatim** (§3.26.15.1) preserving architectural rationale: 4-6 months engineering scope for native-from-day-one (SMTP/IMAP/POP3 protocols + MIME parsing + attachment handling + sandboxed HTML rendering + threading algorithms + spam filtering + deliverability infrastructure + bounce handling + DKIM/SPF/DMARC); ongoing operational infrastructure (24/7 deliverability monitoring + abuse handling + provider reputation management + queue management at scale); reputation risk (Bridgeable's domain reputation directly tied to platform health); regulatory dimensions (CAN-SPAM compliance + GDPR data residency + jurisdictional data sovereignty). The integrate-now-make-native-later commitment is canonical — Bridgeable's email primitive owns its **architectural model** (entity model + UI surfaces + Intelligence integration + operational coupling) while delegating **transport** to provider abstraction (Gmail API + Microsoft Graph + IMAP fallback + transactional providers like Resend). Future native transport implementation lands behind the provider abstraction without disturbing the architectural model.

**Entity model canonical** (§3.26.15.2-§3.26.15.4): four core entities — `email_threads`, `email_messages`, `email_accounts`, `email_user_status`. Threads carry shared metadata (subject + participants list + thread-level operational state); messages carry per-message content (sender + body + attachments + timestamp); accounts carry user's connected provider credentials; user_status carries per-user state (read/unread + snooze + has-task + has-mention) keyed on `(thread_id, user_id)`. Per-user status independence (§3.26.15.13) is load-bearing: two users in the same tenant see different status state for the same thread. Independent state machines.

**Provider abstraction canonical** (§3.26.15.6): `EmailProvider` protocol (similar to Phase D-7 `DeliveryChannel` Protocol) with implementations for Gmail API + MS Graph + IMAP + transactional providers. Each provider implementation handles auth + send + receive + sync. Native transport implementation slots in as an additional provider when ready. Email account links user to provider via OAuth (Gmail + MS Graph) or stored credentials (IMAP); accounts carry per-account configuration (signature + reply-to override + display name).

**HTML body sandbox + outbound sanitization** (§3.26.15.5): inbound HTML messages render in sandboxed iframe (no scripts + no top-level navigation + no popups + sandboxed origin). Inline images resolved via Content-ID lookup against attachment store. Outbound HTML composer output sanitized via DOMPurify-equivalent (no inline scripts + no external CSS + no event handlers); attachments uploaded to R2 via VaultItem creation per §3.26.15.4 attachment-as-VaultItem canonical.

**Storage architecture canonical** (§3.26.15.4-§3.26.15.5): hybrid retention with tenant-configurable extension. Recent message body cached in Bridgeable storage (default 90 days); older messages reference provider via tombstone records (provider message ID + thread ID + minimal metadata) — fetched on-demand from provider for older-than-cache reads. Tenant configuration extends cache window per tenant policy. Attachments are first-class VaultItems per §3.26.15.4 — emails reference the VaultItem; attachment lifecycle (sharing + retention + cross-tenant masking) flows through Bridgeable Vault canonical infrastructure.

**Phase A refinement** (§3.26.15.1 native-vs-integration prose addition): user-directed canonicalization of integrate-now-make-native-later deliberation rationale verbatim so future canon sessions don't re-litigate the trade-off. The 4-6 months scope estimate + ongoing operational infrastructure + reputation risk + regulatory dimensions are explicit canon, not implicit. Bridgeable's email-primitive-as-architectural-model commitment is canonical regardless of transport implementation timing.

### Phase B — Surfacing + Summoning + Composition + Strategic Vision Integration

§3.26.15.10 through §3.26.15.21 + strategic vision integration items.

**Inbox surfacing canonical** (§3.26.15.12): list as default presentation mode (Q5 Phase B leaned-toward) — chronological thread list with per-user status indicators. Alternative presentation modes (kanban by thread state + calendar by message timestamp + grouped by linked entity) deferred per §3.26.7.5. List remains canonical until concrete operator signal warrants alternative defaults.

**Per-message inline reply + modal new/forward** (§3.26.15.13, Q3 Phase B canonical): reply to existing thread happens inline at thread bottom (preserves thread context flow); new email + forward open modal composer (discrete authoring tasks deserve focus-mode treatment). Architectural distinction matters — reply continues an existing conversation, new + forward initiate a new one. Composer surface canon at DESIGN_LANGUAGE §14.9.3.

**Per-user status independence** (§3.26.15.13, Q1 Phase B canonical): per-user status primary; thread-level operational state for shared signals. `email_user_status` table keyed on `(thread_id, user_id)` carries per-user fields (is_read + snoozed_until + has_task + has_mention). Thread-level state carries shared fields (linked_entities + subject + participants). The two state machines compose — per-user status drives inbox display + briefing surfacing; thread-level state drives shared metadata + cross-tenant chrome.

**Cross-tenant native messaging** (§3.26.15.10, kill-the-portal architectural discipline): cross-tenant threads (Hopkins FH ↔ Sunnycrest manufacturing) use Bridgeable-native messaging directly — no external email transport between platform tenants. Cross-tenant threads are first-class platform entities with cross-tenant masking per §3.25.x. Each tenant has its own copy under its own ownership (§3.26.15.18 — independent per-tenant cross-tenant copies); operational state changes don't mirror to the other tenant's copy. Per-tenant copy distinction surfaces in thread footer; chrome looks identical otherwise.

**Retroactive linkage canon detail** (§3.26.15.20, Q4 Phase B canonical): when a partner tenant onboards mid-thread, prior thread copies are explicitly NOT auto-mirrored. Each tenant's copy starts at the moment of cross-tenant relationship establishment. Pre-relationship messages remain in the originating tenant's copy only. Architectural rationale: retroactive auto-mirror across tenant boundaries violates data sovereignty — past messages in the originating tenant predate the cross-tenant relationship and weren't sent under cross-tenant data agreement. The new tenant gets clean cross-tenant continuity from relationship-establishment forward.

**Entity linkage canonical** (§3.26.15.7): emails link to Bridgeable entities (cases + orders + invoices + tasks + contacts). Linkage happens three ways — explicit pre-link at compose time (§14.9.3 modal composer), explicit post-link via per-message action (§14.9.5 affordance chrome), or retroactive via Intelligence detection of recipient → contact → entity-relationship resolution. Linked entities surface in thread Linked Entities panel + drive Pulse activity-stream surfacing per Phase W-4a Layer 1 architecture.

**Front-style shared inbox UX** (§3.26.15.15, strategic vision integration): cross-tenant threads visible to multiple operators within a tenant — shared inbox mental model where operators collaborate on customer conversations. Per-user status independence means each operator's read-state + snooze-state + task-state is independent; thread-level metadata (subject + participants + linked entities) is shared. Front's primary innovation (collaborative customer-facing email without forwarding) is canonical for Bridgeable cross-tenant threads.

**Operational-state-coupled-to-communication** (§3.26.15.16, strategic vision integration): emails carry operational-action affordances binding email content to platform operations. Mark-as-task affordance creates Task linked to message; Link-to-entity affordance binds thread to entity for cross-surface visibility; Generate-state-change affordance surfaces Intelligence-detected candidate state mutations. Email is not a separate inbox island — email content is operational signal that drives platform state when operator confirms.

**State-changes-generate-communications** (§3.26.15.17, strategic vision integration): the inverse coupling — state changes in the platform automatically draft outbound communications. Order status transitions to "shipped" → drafts customer notification email. Case status transitions to "service-scheduled" → drafts FH notification email. Drafted emails surface in Workshop-managed templates (§3.26.15.25 — single email_template type with reply_to_account_id parameter, Q2 Phase C canonical) and require explicit operator confirm before send per §3.26.14.14.5 operator agency discipline.

**Cross-primitive intelligence stream** (§3.26.15.19, strategic vision integration): communications intelligence stream synthesizes across email + SMS + phone + messaging at Pattern C composition. Stream piece narrates per-primitive signal counts plus synthesized cross-primitive insights (e.g., "John Hopkins called twice + emailed once today — escalation pattern"). Stream piece composition canon at DESIGN_LANGUAGE §14.

**Strategic vision deferral catalog** (§3.26.15.21): 25+ deferrals canonicalized including — kanban + calendar presentation modes, real-time collaborative composition, native transport implementation, advanced filtering UI, mass actions, email rules engine, signature management UI, vacation auto-responder, multi-account inbox unified view, threading algorithm tuning, attachment preview enhancements, search ranking refinement, archive policy customization, draft autosave conflict resolution, multi-tenant inbox aggregation, cross-tenant masking presentation refinements, AI compose assistance ("Write a reply"), AI thread summarization, follow-up reminders, recipient suggestions, link-preview rendering, calendar-event-from-email, contact-from-email auto-creation, conversational AI agent over email content, Workshop email-template advanced features (conditional logic + multi-language), Bridgeable Assist email integration. Each deferred per §3.26.7.5 architectural restraint discipline — concrete operator signal triggers future canonicalization.

### Phase C — Focus + Workshop + Intelligence Integration

§3.26.15.22 through §3.26.15.25.

**Email Focus integration** (§3.26.15.22): Email Focus type — Coordination Focus per §3.26.11 Focus Primitive Types. Triggered by: thread escalation (multiple participants + cross-tenant + linked entity) OR explicit summon ("Focus on Hopkins thread"). Participants: thread participants per §3.26.11.9 magic-link participant scope (with cross-tenant masking inherited from §3.25.x). Outcome flow: thread reaches resolution state (all participants marked thread done, OR linked entity reaches terminal state, OR explicit operator close). Auto-closure semantics: Coordination Focus auto-closes when outcome flow completes; Push-back-to-Pulse if reopen needed.

**Workshop email-template integration** (§3.26.15.25, Q2 Phase C canonical): single `email_template` type with `reply_to_account_id` parameter. Workshop manages email templates as templates-as-data per §3.26.14.5; tenants customize platform-default templates via Tune mode (parameter overrides) or fork to tenant-scoped copy via Compose mode. The `reply_to_account_id` parameter resolves to user's connected `email_accounts` per §3.26.15.6 — single template adapts per-user reply-to account routing without proliferation of per-account template copies.

**Intelligence integration** (§3.26.15.23): seven new Intelligence prompts in Bridgeable Intelligence registry — `email.compose.draft_reply` (AI compose assistance), `email.thread.summarize` (thread synthesis), `email.intent.extract_state_change` (state-change candidate detection per §3.26.15.17), `email.entity.detect_linkage` (retroactive linkage suggestion per §3.26.15.7), `email.priority.score` (per-user priority ranking), `email.briefing.synthesize_communications_layer` (Pattern C briefing surface per §3.26.10), `email.cross_primitive.communications_pattern` (cross-primitive intelligence stream per §3.26.15.19). Each prompt seeded as platform-global per Bridgeable Intelligence canonical pattern; tenants customize via Workshop intelligence-prompt template type per §3.26.14.5.

**Cross-references catalog** (§3.26.15.24): comprehensive cross-reference index linking Email primitive to all related canon — Focus types (§3.26.11), Pulse Scope (§3.26.12), Command Bar (§3.26.13), Workshop (§3.26.14), Communications layer (§3.26.9), Briefings (§3.26.10), Bridgeable Intelligence backbone, cross-tenant masking (§3.25.x), operator agency (§3.26.14.14.5), Bridgeable Vault (§5.x).

### Architectural restraint discipline preserved

Per §3.26.7.5 canonical-quality discipline, 25+ explicit deferrals canonicalized (§3.26.15.21 strategic vision deferral catalog). Each deferral carries explicit "concrete signal warrants" trigger. Examples: kanban presentation mode deferred until concrete operator signal warrants alternative default; native transport implementation deferred until provider abstraction matures + operational scale warrants; AI compose assistance deferred until thread-summarization Intelligence ships first as foundation; Bridgeable Assist email integration deferred until Bridgeable Assist primitive itself canonicalizes. The deferral catalog is load-bearing — it preserves deliberation rationale so future sessions don't re-litigate previously-resolved trade-offs.

### Mid-session canon discipline validated: Phase B completion canon hygiene

Phase B initial draft was incomplete — strategic vision integration items (§3.26.15.15-§3.26.15.21) were missing from the first pass. User-flagged via "Phase B draft acknowledged but incomplete... However, the prior turn's prompt directed Phase B canon to include strategic-vision integration. Those items are missing from this draft and need to be added before Phase B closes." Seven additional subsections drafted covering strategic framing + design disciplines + operational coupling + state-changes-generate-communications + Front-style shared inbox UX + cross-tenant native messaging + strategic vision deferral catalog. **Discipline established**: prompts directing comprehensive canon coverage warrant verbatim coverage check before phase close — pre-phase-close hygiene includes "did we hit every item the directing prompt specified?" validation. Pattern reused from Session 1 mid-session reframe (Pulse-fractal insight) and Session 1.5 mid-session primitive emergence (Workshop) — canon discipline allows mid-flight correction rather than locking incomplete canon.

### Canon doc updates locked this session

**BRIDGEABLE_MASTER §3.26.15** introduced as comprehensive Email Primitive section (25 subsections). Phase A: §3.26.15.1 native-vs-integration framework + §3.26.15.2-§3.26.15.4 entity model + §3.26.15.5 HTML sandbox + §3.26.15.6 provider abstraction + §3.26.15.7 entity linkage + §3.26.15.8 attachment-as-VaultItem. Phase B: §3.26.15.10 cross-tenant native messaging + §3.26.15.12 list as default + §3.26.15.13 per-user status + inline reply + modal new/forward + §3.26.15.15 Front-style shared inbox + §3.26.15.16 operational-state-coupled-to-communication + §3.26.15.17 state-changes-generate-communications + §3.26.15.18 independent per-tenant cross-tenant copies + §3.26.15.19 cross-primitive intelligence stream + §3.26.15.20 retroactive linkage canon + §3.26.15.21 strategic vision deferral catalog. Phase C: §3.26.15.22 Email Focus integration + §3.26.15.23 Intelligence integration + §3.26.15.24 cross-references catalog + §3.26.15.25 single email_template type with reply_to_account_id parameter.

**DESIGN_LANGUAGE §14.9** introduced as Email Primitive Visual Canon (5 subsections). §14.9.1 Email inbox visual canon — list item composition + status dot canon + per-user status independence visual + density tier behavior. §14.9.2 Email thread rendering visual canon — thread surface composition + participants section + linked entities section + message rendering canon + long-thread collapse pattern + inline reply composer canon. §14.9.3 Email composition surface visual canon — modal composition surface (Q3 Phase B inline reply + modal new/forward canonical visualization) + account picker + recipient chip input + rich-text composer + attachments. §14.9.4 Cross-tenant thread chrome visual canon — thread-level cross-tenant indicator + cross-tenant masked-field rendering + per-tenant copy distinction. §14.9.5 Operational-action affordance chrome — per-message operational actions + Generate-state-change affordance canon (load-bearing operator agency discipline) + thread-level operational actions. §14.9.6 cross-references.

**AESTHETIC_ARC Session 2 entry** added per drafted prose (this entry). **CLAUDE.md §14 Recent Changes Session 2 entry** added.

### Architectural model post-Session-2

5 primitives + fractal Pulse + universal Command Bar + Workshop + comprehensive Email primitive canon. Email primitive is the **first concrete Communications layer primitive** — Phase W-4b Layer 1 implementation begins after canon foundation complete (Calendar primitive next, ~3-4 weeks parallel-able with Email implementation per Phase W-4b sequence step 2). Architectural elegance achievement: every email-specific surface unifies under existing primitive framework (Pulse for surfacing + Command Bar for summoning + Focus for coordination + Workshop for templates + Intelligence for synthesis); no speculative parallel primitives.

### Canon-quality status post-Session-2

Sessions 1 + 1.5 + 2 collectively canonicalize: Focus Primitive Types + Pulse Scope Architecture + Command Bar Architecture + Workshop Primitive + Email Primitive. Six explicit deferrals from Sessions 1 + 1.5 + 25+ from Session 2 tracked. Phase W-4b Layer 1 Email implementation unblocked; Calendar canon (Session 3 candidate) + SMS canon (Session 4 candidate) + Phone canon (Session 5 candidate) + Messaging canon (Session 6 candidate) sequence forward as Phase W-4b implementation arc proceeds.

**No code shipped this session.** Architecturally locked foundation for Phase W-4b Layer 1 Email primitive implementation; integrate-now-make-native-later framework established; templates-as-data discipline reinforced (Workshop email-template type); operator agency discipline reinforced (Generate-state-change affordance never auto-applies); kill-the-portal discipline reinforced (cross-tenant native messaging direct). Implementation deferred to scoped session(s) starting with Phase W-4b Layer 1 Email primitive build per BRIDGEABLE_MASTER §3.26.6.4 sequence step 1.
...
- **2026-05-04 (Phase W-4a Step 6 — Pulse Viewport-Responsive Architecture complete):** Six-commit implementation arc closes. The viewport-fit math foundation (Commit 1), tier-based columns + container-query density tiers (Commit 2), scroll mode + tier-three threshold + sub-pixel-and-border boundary buffer (Commit 3), empty slot filter + CI parity test (Commit 4), workspace-core scroll-within-piece canon-conformance (Commit 5), and §13.8 reference implementations across 5 viewport sizes (Commit 6) ship as a coherent architectural unit. **Cumulative**: 17 net new files + ~17 modified files, ~5,200 LOC, 97 new tests, 871/871 vitest. **10 viewport-matrix screenshots** captured under `docs/design-references/pulse-step-6/` (mobile / tablet / desktop-common / desktop-FHD / 4K × light/dark) — these are the **calibration anchors** for subsequent Pulse component refinement, post-September iteration, and future widget development. When a widget's rendering at any of these viewports drifts from the locked references, that's a regression to investigate. **DESIGN_LANGUAGE §13.8 augmented** with 7 new subsections (13.8.9–13.8.15) preserving existing 13.8.1–13.8.8 W-4a foundation references; cross-references between §13.3.1 / §13.3.4 / §13.4.1 / §13.4.3 / §13.8 + BRIDGEABLE_MASTER §3.26 verified bidirectional. Phase W-4a ships canonically; Phase W-4b implementation arc unblocked beginning with Layer 1 email primitive.
